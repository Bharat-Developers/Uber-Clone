...
export const GET = async (request: Request) => {
  ...
  if (id) {
    try {
      ...
      return new NextResponse(
        JSON.stringify({ message: "Error Getting Driver" }),
        { status: 500 }
      );
    }
  }
...
export const DELETE = async (request: Request) => {
  try {
    ...
    if (!id) {
      return new NextResponse(
        JSON.stringify({ message: "ID needed to Delete Driver" }),
        { status: 404 }
      );
    }
    await connect();
    ...
    if (!driver) {
      return new NextResponse(JSON.stringify({ message: "Driver not found" }), {
        status: 404,
      });
    }
    return new NextResponse(
    ...
    return new NextResponse(
      JSON.stringify({ message: "Error in Deleting Driver" }),
      { status: 500 }
    );
  }
};
...
export const PUT = async (request: Request) => {
  try {
    ...
    if (!id) {
      return new NextResponse(
        JSON.stringify({ message: "ID needed to Update Driver" }),
        { status: 404 }
      );
    }
    const body = await request.json();
    ...
    return new NextResponse(
      JSON.stringify({ message: "Driver Updated Successfully" }),
      { status: 201 }
    );
  } catch (error) {
    return new NextResponse(
      JSON.stringify({ message: "Error in Updating Driver" }),
      { status: 500 }
    );
  }
};
